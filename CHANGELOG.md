...
* **技術方案與關鍵決策**（How）：
  ...
  2. 在打包步驟中將產出的 APK 額外複製正名為 `AzaharPlus-zh.apk`。
  3. 整合 `softprops/action-gh-release@v2`，當 `main-zh` 建置成功時，自動發布標籤為 `v2126-zh` 的正式 Release，並將 APK 直接附加於 Release 資源中供直接下載。
...
* **修改日期**：2026-09-20
* **目標分支**：`main-zh`
* **修改分類**：`[在地化精修 / 台灣繁體中文標準 / 陣列補齊]`
* **涉及檔案清單**：
  * 修改：`src/android/app/src/main/res/values-b+zh+TW/strings.xml`（更正台灣地區名稱為「台灣」、更正香港地區名稱為「香港」、更正澳門地區名稱為「澳門」、「朝鮮語」更正為「韓語」）
  * 新增：`src/android/app/src/main/res/values-b+zh+TW/arrays.xml`（補齊 9 個靜態主題顏色之繁中翻譯）
  * 修改：`CHANGELOG.md`（記錄變更日誌與索引追溯）
* **修改動機與問題**（Why）：
  * 深度清查發現國家地區清單中的 `taiwan` 原本標記錯誤，且語言名稱中出現非台灣慣用之「朝鮮語」。
  * 原專案未提供繁中 `arrays.xml`，導致靜態主題顏色選單（`staticThemeNames`）仍為英文。
* **技術方案與關鍵決策**（How）：
  1. 遵照使用者指令與全域規範，將 `taiwan` 嚴格修正為「**台灣**」，徹底杜絕不當稱謂；同步將「朝鮮語」修正為台灣常用之「**韓語**」。
  2. 建立繁體中文專屬之 `values-b+zh+TW/arrays.xml`，將靜態色彩主題完整在地化（藍色 (預設)、青色、紅色、綠色、黃色、橘色、紫羅蘭色、粉紅色、灰色）。
* **測試與驗證結果**（Verification）：
  * XML 結構與解析測試全數通過，無任何殘留之不當稱謂。
